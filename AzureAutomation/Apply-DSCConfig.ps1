...
$PublicFQDN = 'iton.nl'
$DomainName = "demo2016.$($PublicFQDN)"
$DomainNetbiosName = $CustomerName
$RDGatewayExternalFqdn = "$($CustomerShortName)-rdg.$($PublicFQDN)"
...
$DNSServerName = "$($CustomerShortName)-cc" #Is dit nodig? Kan misschien ook $DNSIPAddress gebruikt worden?
$RDLicenseServer = "$($CustomerShortName)-cc"

#Create DN based upon domainname
$DistinguishedName = [string]::Empty
$collection = $DomainName.split('.')
foreach ($item in $collection){
    $DistinguishedName += ",DC=$($item)"
}
...
$ParamSplat = @{
    ...
    ADFSAccountPassword = $ADFSAccountPassword
    ADFSPublicFQDN = $ADFSPublicFQDN
    RDBrokerPublicFQDN = $RDBrokerPublicFQDN
    ADFSDisplayName = "Welkom bij $($CustomerName)"
}
...
$NodeName = "$($CustomerShortName)-rdgw1"
...
$ParamSplat = @{
    ...
    OUPath = "OU=Computers,OU=$($EntityName),$($DistinguishedName)"
    ConnectionBroker = $ConnectionBroker
    RDGatewayExternalFqdn = $RDGatewayExternalFqdn
    RDPortalName = $RDPortalName
    RDBrokerExternalFQDN = $RDBrokerExternalFQDN
...
$ParamSplat = @{
        ...
        OUPath = "OU=Computers,OU=$($EntityName),$($DistinguishedName)"
        ConnectionBroker = $ConnectionBroker
        RDLicenseServer = $RDLicenseServer
}
